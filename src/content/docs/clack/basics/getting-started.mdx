...
You can install Clack using npm, pnpm, Yarn, or Deno:
...
<Aside type="note">On Deno, Clack reads environment variables such as `TERM` to detect terminal capabilities, so your CLI needs the `--allow-env` (`-E`) permission.</Aside>
